...
export default function TradingPage() {
  ...
  const runAnalysis = useCallback(async (symbol: string) => {
    ...
    try {
      ...
      if (signal) {
        ...
        s.setAnalysisError("AI response could not be parsed. Try re-running.");
      }
    } catch (err) {
  ...
  // Poll the quote every 30s for the selected symbol.
  useEffect(() => {
    if (!s.symbol) return;
          ...
          <div className="glass-card p-4">
            ...
            {s.loadingQuote && !quote ? (
              ...
              <div className="py-8 text-center text-white/45">
                Search for a stock above to begin analysis.
              </div>
            )}
